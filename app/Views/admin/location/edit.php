...
<div class="mb-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Update location information</p>
        </div>
...
<div class="bg-white rounded-lg shadow-md p-6">
    <form action="<?= base_url('admin/location/update/' . $location['id']) ?>" method="post" id="locationForm">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text"
                    id="departemen"
                    name="departemen"
                    value="<?= old('departemen', $location['departemen']) ?>"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
            ...
            <div>
                ...
                <select id="status"
                    ...
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    required>
                    <option value="aktif" <?= old('status', $location['status']) == 'aktif' ? 'selected' : '' ?>>Active</option>
                    <option value="tidak_aktif" <?= old('status', $location['status']) == 'tidak_aktif' ? 'selected' : '' ?>>Inactive</option>
                </select>
        ...
        <div class="mt-6">
            ...
            <textarea id="keterangan"
                ...
                rows="3"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                placeholder="Additional information about this location"><?= old('keterangan', $location['keterangan']) ?></textarea>
